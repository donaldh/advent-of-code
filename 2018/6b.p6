...
my @coords = '6-input.txt'.IO.lines.map: {
    my Int ($x, $y) = .comb(/\d+/)>>.Int;
    Coord.new(:$x, :$y);
};

my Int ($min-x, $max-x) = @coords.min(*.x).x, @coords.max(*.x).x;
my Int ($min-y, $max-y) = @coords.min(*.y).y, @coords.max(*.y).y;
